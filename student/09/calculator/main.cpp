...
int main() {
    ...
    while ( true ) {
        ...
        std::transform(command_to_be_executed.begin(),
                       command_to_be_executed.end(),
                       command_to_be_executed.begin(),
                       ::toupper);
        ...
        for(Command const &command : COMMANDS){
            if(command_to_be_executed == command.str){
                command_not_found = false;                if(pieces.size()-1 != command.parameter_number){
                ...
                if (string_to_double(pieces.at(1), param1) && string_to_double(pieces.at(2), param2)){
                    std::cout<< command.action(param1, param2)<<std::endl;
                    break;
                }
