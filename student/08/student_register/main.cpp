...
void write_new_data(const std::string& file_name,
                    std::map< std::string, Student* >& alphabetical_order
                    ){
    std::ofstream file_object(file_name);
    for (auto itr = alphabetical_order.begin(); itr!=alphabetical_order.end(); itr++){
        std::string line = "";
        line += itr->second->student_number;
        line += ";";
        std::string field = itr->first;
        line = line + field + ";";
        line += itr->second->name;
        line += ";";
        line += itr->second->phone_number;
        line += ";";
        line += itr->second->email;
        line += ";";
        line += itr->second->skype;
        file_object << line << std::endl;
    }
    file_object.close();
}
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                if (student_numbers.find(parts.at(1)) != student_numbers.end()){
                    ...
                    std::getline(std::cin, phone_number_new);
                    std::cout << std::endl;
                    if (not is_valid_phone_number(phone_number_new)){
                        continue;
                    }
                    else{
                        student_numbers[parts.at(1)]->phone_number = phone_number_new;
                        //write new data in txt file
                        write_new_data(file_name, user_ids);
                    }
                }
                else{
